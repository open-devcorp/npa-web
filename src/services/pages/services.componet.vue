<template>
  <div class="bg-noise-white">
    ...
    <section class="py-4 max-w-screen-xl mx-auto px-4 gap-32 mt-10">
      ...
      <!-- Card selector -->
      <div class="flex flex-col md:flex-row  md:h-[320px] lg:h-[400px] overflow-hidden gap-3 md:gap-4 mt-4"
        ...
        <div v-for="(card, index) in cards" :key="index"
          ...
          }" @mouseenter="selectCard(index)" @click="selectCard(index)">
          <template v-if="card.img">
            <img :src="card.img" :alt="card.alt"
              class="w-full h-full object-cover object-center transition-transform duration-300" />
          </template>

          <div v-if="selected !== index" class="absolute inset-0 bg-tertiary/65 z-10 flex items-center justify-center">
